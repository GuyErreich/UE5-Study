...
void UQuickAssetAction::DuplicateAsset(int32 numOfDuplicates)
{
   ...
   {
      ShowMsgDialog(EAppMsgType::Ok, "Please enter a valid number", true);
      return;
   }
   ...
   if (counter > 0)
      ShowNotifyInfo("Successfully duplicated asset " + FString::FromInt(counter) + " files");
}
...
void UQuickAssetAction::AddPrefixes()
{
   ...
   for (UObject* selectedObject : selectedObjects)
   {
      ...
      {
         ShowMsgDialog(EAppMsgType::Ok, "Failed to find prefix for class " + selectedObject->GetClass()->GetName(), true);
         continue;
      }
      ...
      {
         ShowNotifyInfo(oldName + " already has prefix added");
         continue;
      }
...
void UQuickAssetAction::RemoveUnusedAssets()
{
   ...
   {
      ShowMsgDialog(EAppMsgType::Ok, "No unused asset found among selected assets", false);
      return;
   }
   ...
   if (!isDeleted)
   {
      ShowMsgDialog(EAppMsgType::Ok, "No unused asset was deleted", false);
      return;
   }

   ShowNotifyInfo("All unused assets have been deleted");
}
